...
<br/>
 <form action="buyfromagency.htm" method="post" onsubmit="return validate(this);">
<!-- row  -->
<div class="row">
        <div class="col-md-9">
		...
		$('#divNoLocks').css("display", "none"); 
			...
			return false;
				...
				$('#addMachineOrder').click(
					...
					var totalPrice=parseFloat(finalPrice)*parseInt(quantity);
				...
				$("#txtTotalCost").val(tcost);
					...
					for(var index=0;index<tbodyRows;index++)
					{
						...
						if(kind==tkind && model==tmodel)
						{
							...
							quantity=parseInt(quantity)+parseInt(tableRow.find('td').eq(3).text());
							totalPrice=parseFloat(totalPrice)+parseFloat(tableRow.find('td').eq(4).text());
							isNotExist=false;
							break;
				...
				$("#txtTotalCost").val(tcost);
					...
					for(var index=0;index<tbodyRows;index++)
					{
						...
						if(kind==tkind && model==tmodel)
						{
							...
							quantity=parseInt(quantity)+parseInt(tableRow.find('td').eq(3).text());
							totalPrice=parseFloat(totalPrice)+parseFloat(tableRow.find('td').eq(2).text());
							isNotExist=false;
							break;
